...
You're picking up `siglip2.cpp` after the 2026-05-07 VRAM pass + 2026-05-08 perf pass + 2026-05-09 QKV-fusion pass + 2026-05-09 megakernel-v0 Phase A0 (fused LayerNorm) + 2026-05-09 megakernel-v0 Phase A1 (QKV-prep, 2-kernel scratch). Feature parity has been complete since M5; the prior passes drove VRAM from 1855 MiB to 1438 MiB and added padded MMA-FA for d_head=72; QKV fusion closed 14 % off the text path; Phase A0 introduced the `src/cuda/` scaffolding with fused LN; Phase A1 layered on the QKV-prep chain fusion (9 ops/block → 2 kernels) for another **9-11 %** off every endpoint, total **12-15 %** since pre-megakernel. Read `AGENTS.md` for the architectural overview; this doc is **what to do next + why + the user's disposition**.

## Cold-pickup orientation — if you've got a long run ahead

The user is asleep. They want to come back to "shocking news," not to a polite request for permission. **Phase A0 + A1 of the megakernel plan have landed this pass** — fused LayerNorm and 2-kernel scratch QKV-prep — for **12-15 % per endpoint** off the pre-megakernel baseline. Where to go next, in order of where I'd put my chips:

1. **A python re-bench.** The pre-megakernel "cpp 1.43× python" / "cpp 1.79× python" ratios are stale. With A0+A1 we're at 33.5 ms text and 33.6 ms classify_from_embeddings on a clean GPU, and the python p50 was 26 ms / 21 ms — so the gap has likely closed to roughly 1.29× / 1.60×. Run `/tmp/bench_cpp.py http://localhost:8890` (kobbler-vision-1) clean-GPU to get the current baseline before chasing more fusions.

2. **Phase A2 — fuse the post-FA `cont` into the o_proj input prep.** After flash_attn_ext, build_block does `view_3d(slice off pad) → cont` to convert FA's (d_pad, n_head, n_pos) output back to the (H, n_pos) input the o_proj mul_mat expects. That's 1 launch per block × 27 = 27 launches, ~0.3 ms. Same per-op hook scaffolding as A0/A1, simpler kernel (no pad/cast — just F32 stride conversion). Modest but free with the existing scaffolding.

3. **Phase B — fuse `mul_mat + bias_add` for the 4 mul_mat-bias pairs per block (qkv, o, up, down)**. Requires writing custom Q8_0×F32 mul_mat kernels (M=64 for text, M=729 for vision) that fold the bias into the accumulator. Big lift — qwen3-tts only ships M=1 specialized MMVQ; siglip2 needs MMQ-class. ~135 launches/encode saved, ~1.5 ms. Worth doing but not a one-session swing. See `HANDOFF-megakernel-v0.md` "what's next post-A1" for entry points.

4. **Graph caching for fixed-shape encoders.** Pre-megakernel "Dead ends" still applies — both shared-sched and per-entry-sched hit gallocr offset reuse on call 2. The CUDA-graphs-on-megakernel idea is much more attractive now that the inner kernel surface is smaller and shape-fixed. After A2, every block's launch sequence is deterministic per shape; CUDA graph capture should fire after the warmup window. ggml's gallocr is the bottleneck; either fork it for a re-bind entry point or skip the scheduler entirely (raw `ggml_backend_graph_compute` with one pinned compute buffer). Estimated 10-20 % more on text once it lands.
...
- Endpoint p50 (30 runs). The 2026-05-09 measurement protocol is **one tower at a time** — kobbler-vision-1 doesn't unload itself and pollutes side-by-side numbers. The "clean GPU" column below is C++ alone (kobbler-vision-1 stopped) post-A0+A1.
  - `/v1/embeddings`:               clean **49.6 ms** (was 58.2 pre-A0; **−14.8 %** total — A0 took 4.1 %, A1 the other 11.3 %)
  - `/v1/text_embeddings`:          clean **33.5 ms** (was 38.3 pre-A0; **−12.5 %**)
  - `/v1/classify`:                 clean **80.8 ms** (was 94.1 pre-A0; **−14.1 %**)
  - `/v1/classify_from_embeddings`: clean **33.6 ms** (was 38.5 pre-A0; **−12.7 %**)
  - Python head-to-head re-bench is the next obvious thing — pre-megakernel ratios were 1.07× / 1.43× / 1.34× / 1.79× python under shared-GPU contention. After ~13 % wins per endpoint, expectations are roughly cpp 0.95× / 1.25× / 1.20× / 1.55× python, but actually-measured needs a clean-GPU vs Python run with kobbler-vision-1 alone.
